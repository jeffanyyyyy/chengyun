...
整頁的顏色都取自宣傳海報 `assets/poster.jpg`，集中定義在 `index.html` 最上方的 `:root`：

| 變數 | 色值 | 用途 |
| --- | --- | --- |
| `--earth` | `#5f3a1c` | 海報標題的深棕，用於標題、頁尾、深色區塊 |
| `--clay` | `#c8763a` | 海報筆刷的陶土橘，用於重點與分隔線 |
| `--clay-deep` | `#a2551f` | 按鈕與連結（深一階，確保白字對比達 AA） |
| `--cream` / `--paper` | `#f5efe1` / `#fffaf0` | 海報的暖米紙底與卡片底色 |
| `--jade` | `#2f6b63` | 標誌的青綠，用於步驟編號與 Q&A 的 A 標記 |

卡片圓角統一用 `--r-lg` / `--r-md` / `--r-sm`，對應海報上的大圓角方塊。
要換色只要改 `:root` 的數值，全頁會一起跟著變。
